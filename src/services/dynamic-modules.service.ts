...
export class DynamicModulesService {
  async createModule(modelName: string, fields: any[]) {
    const srcDir = join(process.cwd(), 'src', 'modules', modelName)
  ...
  private generateDtoField = (field: FieldsInputs, type: 'input' | 'dto') => {
...
export const dynamicModules = [
  ${modelName}Module,
    ...
    const importStatement = `import { ${modelName}Module } from './modules/${modelName}/${modelName}.module';`
